defmodule Vutuv.SocialFeed.PostTruncateTest do
  @moduledoc """
  The one clamp in the remote-content stack (issue #1741).

  Its own file, and `async: true`, because it is a pure string function that
  six modules across three subsystems now share — a remote post's body
  (`Vutuv.Mastodon`, `Vutuv.Bluesky`), the reduced text of a stranger's HTML
  (`Vutuv.RemoteHtml`), a card's headline and teaser (`Vutuv.OpenGraph`,
  `Vutuv.LinkSummary`) and a repository description (`Vutuv.CodeStats.Snapshot`).
  It used to be two implementations that disagreed, so what it promises is worth
  stating in one place rather than inferring from whichever caller you happen to
  be reading.
  """
  ...
  test "the last run of non-space goes even when it happened to fit exactly" do
    ...
    # landing exactly on a word boundary still drops that word. That is the
    # behaviour `Vutuv.LinkSummary` shipped and it is kept deliberately: the
    # alternative is peeking at the character past the budget to tell "ended
    # cleanly" from "cut mid-word", and one spare word costs less than a rule
    # with two branches. Recorded here so the next reader knows it was a
    # choice.
