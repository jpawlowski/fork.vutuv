defmodule Vutuv.OpenGraph do
  ...
  **The parser is deliberately not an HTML parser.** All that is needed are the
  ...
  their attributes with a regex, and the HTML entities in the values are decoded
  (`Vutuv.RemoteHtml.decode_entities/1`). Only the first
  ...
  **A usable `:image_url` is not required.** A page that declares `og:image`
  ...
  # `scrub_nul/1` before anything else: a page may put a literal NUL byte in a
  # `content` attribute or a `<title>`, which is not an entity, so the decoder's
  # own refusal never sees it — and `\s` does not match it either. It would ride
  # all the way into `Screenshots.mark_ready/2`'s update and raise there.
  defp normalise(value, max) do
    value
    |> RemoteHtml.scrub_nul()
    |> RemoteHtml.decode_entities()
