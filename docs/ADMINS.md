...
| `CARDDAV_ENABLED` | `true` | `false` turns the read-only CardDAV address book off (`/.well-known/carddav` and the whole collection 404). On by default and it makes no outbound calls at all, so an intranet installation has no reason to switch it off — the switch is for an operator who does not want members' contacts leaving for a phone in the first place. It is **off per member** either way: each member picks on `/settings/carddav` whether they publish nobody (the default), only contacts they marked as personally known, only mutual follows, or everybody they follow. The password a device stores is a personal access token with the `contacts:read` scope, never the account password, and it is revocable per device under `/settings/apps`. Devices that speak **WebDAV-Push** (DAVx⁵) are told when a member's address book changes instead of polling for it; that rides on Web Push, so it follows `WEB_PUSH_ENABLED` and needs no switch of its own. iOS and macOS Contacts use Apple's own APNS-based push extension, which needs a certificate from Apple that no third-party server can obtain — those clients poll. See [carddav.md](architecture/carddav.md) |
...
- Set `WEB_PUSH_ENABLED=false`: Web Push needs no configuration and is
  ...
  keeps working.
- Optional DNS, if you want members to be able to type nothing but your host
  name into a phone: `/.well-known/carddav` (RFC 6764) is served out of the box
  and is what a client tries second. What it tries **first** is an SRV record,
  so adding these makes the bare host name work everywhere and costs nothing:
  `_carddavs._tcp.<your host>. SRV 0 1 443 <your host>.` (use `_carddav._tcp`
  and port 80 on an installation without TLS). Skip them and members type the
  full `https://<your host>` instead, which every client accepts.
- WebDAV-Push works on an intranet too: the notification goes to whatever push
  service the *device* registered, so on a fully air-gapped network a client
  using a self-hosted UnifiedPush distributor is fine, and one relying on
  Google's FCM simply never gets a push and falls back to polling. Nothing is
  sent anywhere the device did not name itself.
- `CARDDAV_ENABLED` can stay on: the address book is served from your own
  server to devices on your own network and calls nothing out. On an intranet
  it is one of the more useful things vutuv does — the company directory lands
  in everybody's Contacts app.
