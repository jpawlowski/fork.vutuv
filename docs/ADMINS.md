...
| `SUMMARIZE_LINKS` | `false` | `true` gives a link preview a hover tooltip saying what the linked page is about — one sentence, at most 200 characters, written by a local Ollama text model from the **whole** page rather than from the part the screenshot shows. It is written after the screenshot is already stored and shown, once per capture, and never retried: with the flag off, no Ollama, or a page that answers nothing readable, the preview stays exactly as it is and simply has no tooltip. One answer gets 30 seconds, which is not a knob. Leave off on installations without an Ollama that can carry the model |
