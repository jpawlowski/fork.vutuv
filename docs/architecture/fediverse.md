...
warning** gets the same automatic link preview a member's post gets — one shared
...
YouTube thumbnail branch, Open Graph branch, retries, `:generate_screenshots`
gate, the
...
do), and the card renders the preview exactly as a member post's card does —
the page's own Open Graph card full width below the text, a bare screenshot
floated beside it.
